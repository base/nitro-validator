 ...
 */
library ECDSA384Hinted {
    using MemoryUtils for *;
    using U384Hinted for *;
    ...
    function _verify(
        ...
        unchecked {
            ...
            (inputs_.r, inputs_.s) = U384Hinted.init2(signature_);
            (inputs_.x, inputs_.y) = U384Hinted.init2(pubKey_);
            ...
            uint256 call =
                useHints_ ? U384Hinted.initCallWithHints(params_.p, inverseHints_) : U384Hinted.initCall(params_.p);

            /// accept s only from the lower part of the curve
            if (
                U384Hinted.eqInteger(inputs_.r, 0) || U384Hinted.cmp(inputs_.r, params_.n) >= 0
                    || U384Hinted.eqInteger(inputs_.s, 0) || U384Hinted.cmp(inputs_.s, params_.lowSmax) > 0
            ) {
                return (false, U384Hinted.hintCursor(call));
            }
            ...
            uint256 scalar1 = U384Hinted.moddiv(call, hashedMessage_.init(), inputs_.s, params_.n);
            uint256 scalar2 = U384Hinted.moddiv(call, inputs_.r, inputs_.s, params_.n);

            {
                uint256 three = U384Hinted.init(3);
            ...
            U384Hinted.modAssign(call, scalar1, params_.n);

            return (U384Hinted.eq(scalar1, inputs_.r), U384Hinted.hintCursor(call));
        }
    }
    ...
    function _isOnCurve(uint256 call, uint256 p, uint256 a, uint256 b, uint256 x, uint256 y) private returns (bool) {
        unchecked {
            if (U384Hinted.eqInteger(x, 0) || U384Hinted.eq(x, p) || U384Hinted.eqInteger(y, 0) || U384Hinted.eq(y, p))
            {
                return false;
            }

            uint256 LHS = U384Hinted.modexp(call, y, 2);
            uint256 RHS = U384Hinted.modexp(call, x, 3);

            if (!U384Hinted.eqInteger(a, 0)) {
                RHS = U384Hinted.modadd(RHS, U384Hinted.modmul(call, x, a), p); // x^3 + a*x
            }

            if (!U384Hinted.eqInteger(b, 0)) {
                RHS = U384Hinted.modadd(RHS, b, p); // x^3 + a*x + b
            }

            return U384Hinted.eq(LHS, RHS);
        }
    }
        ...
        returns (uint256 x2, uint256 y2)
    {
        unchecked {
            ...
            if (U384Hinted.eqInteger(y1, 0)) {
                return (0, 0);
            }

            uint256 m1 = U384Hinted.modexp(call, x1, 2);
            U384Hinted.modmulAssign(call, m1, three);
            U384Hinted.modaddAssign(m1, a, p);

            uint256 m2 = U384Hinted.modshl1(y1, p);
            U384Hinted.moddivAssign(call, m1, m2);

            x2 = U384Hinted.modexp(call, m1, 2);
            U384Hinted.modsubAssign(x2, x1, p);
            U384Hinted.modsubAssign(x2, x1, p);

            y2 = U384Hinted.modsub(x1, x2, p);
            U384Hinted.modmulAssign(call, y2, m1);
            U384Hinted.modsubAssign(y2, y1, p);
        }
    }
        ...
        returns (uint256 x2, uint256 y2)
    {
        unchecked {
            ...
            if (U384Hinted.eqInteger(y2, 0)) {
                return (0, 0);
            }

            U384Hinted.modexpAssignTo(call, m1, x2, 2);
            U384Hinted.modmulAssign(call, m1, three);
            U384Hinted.modaddAssign(m1, a, p);

            U384Hinted.modshl1AssignTo(m2, y2, p);
            U384Hinted.moddivAssign(call, m1, m2);

            U384Hinted.modexpAssignTo(call, x1, m1, 2);
            U384Hinted.modsubAssign(x1, x2, p);
            U384Hinted.modsubAssign(x1, x2, p);

            U384Hinted.modsubAssignTo(y1, x2, x1, p);
            U384Hinted.modmulAssign(call, y1, m1);
            U384Hinted.modsubAssign(y1, y2, p);

            if (U384Hinted.eqInteger(y1, 0)) {
                return (0, 0);
            }

            U384Hinted.modexpAssignTo(call, m1, x1, 2);
            U384Hinted.modmulAssign(call, m1, three);
            U384Hinted.modaddAssign(m1, a, p);

            U384Hinted.modshl1AssignTo(m2, y1, p);
            U384Hinted.moddivAssign(call, m1, m2);

            U384Hinted.modexpAssignTo(call, x2, m1, 2);
            U384Hinted.modsubAssign(x2, x1, p);
            U384Hinted.modsubAssign(x2, x1, p);

            U384Hinted.modsubAssignTo(y2, x1, x2, p);
            U384Hinted.modmulAssign(call, y2, m1);
            U384Hinted.modsubAssign(y2, y1, p);
        }
    }
    ...
    function _addAffine(
        ...
        unchecked {
            ...
            if (U384Hinted.eq(x1, x2)) {
                if (U384Hinted.eq(y1, y2)) {
            ...
            uint256 m1 = U384Hinted.modsub(y1, y2, p);
            uint256 m2 = U384Hinted.modsub(x1, x2, p);

            U384Hinted.moddivAssign(call, m1, m2);

            x3 = U384Hinted.modexp(call, m1, 2);
            U384Hinted.modsubAssign(x3, x1, p);
            U384Hinted.modsubAssign(x3, x2, p);

            y3 = U384Hinted.modsub(x1, x3, p);
            U384Hinted.modmulAssign(call, y3, m1);
            U384Hinted.modsubAssign(y3, y1, p);
        }
    }
