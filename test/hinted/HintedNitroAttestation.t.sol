...
contract U384TestWrapper {
    ...
    function computeModinvNoHint(bytes memory bBytes, bytes memory mBytes)
        external
        view
    ...
    function callModdivAssignWithHint(
        bytes memory aBytes,
        bytes memory bBytes,
        bytes memory pBytes,
        bytes memory hint
...
contract HintedNitroAttestationTest is Test {
    ...
    ///      acceptance: empty / garbage / the original-valid / padded hints all revert. `s`
    ///      is corrupted (not `r`) so range checks pass and the first scalar inversion's hint
    ///      check is the gate that fires.
    function test_CorruptedSignatureNeverAccepts() public {
        ...
        corrupted[48] = bytes1(uint8(corrupted[48]) ^ 0x01); // flip MSB of s
